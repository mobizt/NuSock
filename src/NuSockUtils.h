...
public:
    static size_t encode(const uint8_t* data, size_t length, char* outBuffer, size_t outMaxLen) {
        ...
        for (size_t i = 0; i < length; i++) {
            ...
            valb += 8;
            while (valb >= 0) {
                if (outLen < outMaxLen - 1) outBuffer[outLen++] = table[(val >> valb) & 0x3F];
                valb -= 6;
            }
        }
        if (valb > -6) {
            if (outLen < outMaxLen - 1) outBuffer[outLen++] = table[((val << 8) >> (valb + 8)) & 0x3F];
        }
        while (outLen % 4) {
            if (outLen < outMaxLen - 1) outBuffer[outLen++] = '=';
        }
        outBuffer[outLen] = 0;
        return outLen;
    }
...
public:
    ...
    void final(uint8_t digest[20]) {
        ...
        for (i = 0; i < 8; i++) finalcount[i] = (unsigned char)((count[(i >= 4 ? 0 : 1)] >> ((3 - (i & 3)) * 8)) & 255);
        update((uint8_t *)"\200", 1);
        while ((count[0] & 504) != 448) update((uint8_t *)"\0", 1);
        update(finalcount, 8);
        for (i = 0; i < 20; i++) digest[i] = (unsigned char)((state[i >> 2] >> ((3 - (i & 3)) * 8)) & 255);
    }
};
